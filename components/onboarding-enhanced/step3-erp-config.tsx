...
import type { ErpConfiguration, ERPTemplate } from "@/types"
import { getERPTemplates } from "@/lib/erp-templates"
// Add import for validation
import { validateERPConfiguration } from "@/lib/validation-utils"

interface Step3Props {
  erpConfig?: ErpConfiguration
  tenantId: string // Add tenantId prop
  onNext: (erpConfig: ErpConfiguration) => void
  onBack: () => void
  saving?: boolean
}

export default function Step3ERPConfig({ erpConfig, tenantId, onNext, onBack, saving = false }: Step3Props) {
  const [templates, setTemplates] = useState<ERPTemplate[]>([])
  const [selectedTemplate, setSelectedTemplate] = useState<string>(erpConfig?.template_id || "")
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (template) {
      const erpConfigData: ErpConfiguration = {
        id: erpConfig?.id || undefined, // Include existing ID if available
        tenant_id: tenantId, // Use the tenantId prop
        template_id: template.id,
        erp_template_name: template.name, // Use erp_template_name as per type
        fields,
        enabled_commands: template.commands.map((c) => c.name),
        connection_status: "pending",
        created_at: erpConfig?.created_at || new Date().toISOString(),
        updated_at: new Date().toISOString(),
      }

      // Validate the ERP configuration before proceeding
      const validation = await validateERPConfiguration(erpConfigData)

      if (!validation.isValid) {
        console.error("ERP validation failed:", validation.errors)
        alert(`Erro na validação: ${validation.errors.join(", ")}`)
        return
      }

      if (validation.warnings.length > 0) {
        console.warn("ERP validation warnings:", validation.warnings)
      }

      onNext(erpConfigData)
    }
  }
