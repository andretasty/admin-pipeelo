...
// --- TENANTS CRUD ---
export async function saveTenant(tenant: Tenant, address?: Address): Promise<{ success: boolean; data?: Tenant; error?: string }> {
  try {
...
export async function saveErpConfiguration(config: ErpConfiguration): Promise<{ success: boolean; data?: ErpConfiguration; error?: string }> {
  try {
    const id = config.id || generateUUID();
...
// --- ASSISTANTS CRUD ---
export async function saveAssistant(assistant: Assistant): Promise<{ success: boolean; data?: Assistant; error?: string }> {
  try {
